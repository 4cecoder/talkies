...
S1-mini model output handling is also defined by the shared
[`s1-mini-cleanup-result-golden.json`](../../linux/src/testdata/s1-mini-cleanup-result-golden.json).
The macOS, Windows, and Linux suites verify the same trimming and empty-output
fallback behavior, so an empty local model response cannot silently erase ASR
text on one desktop platform.
