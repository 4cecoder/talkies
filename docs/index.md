...
The macOS package builds with Swift 6.3.3. Stable `TalkiesCore` and volatile `TalkiesInference` are separate dynamic products embedded in the app bundle; `TalkiesAudio` owns AVFoundation capture and device selection. The release packager checks the executable's dylib links and signs nested code before the app. S1-mini cleanup is opt-in in General settings and runs on the CPU. Windows and Linux also have local S1-mini implementations with pinned weights, model integrity checks, and raw-ASR fallback. Each platform has a native UI and model lifecycle; shared acceptance coverage and complete feature parity remain in progress. After the models are available, dictation and transcript cleanup run offline and transcript text stays on the device.
