...
The macOS app has a menu-bar entry, a floating dictation window, selectable Left or Right Option activation with press/hold detection, AVFoundation recording, WhisperKit transcription with local personal-vocabulary hints, cursor insertion with asynchronous paste and multi-representation clipboard restoration, opt-in in-process S1-mini cleanup through llama.cpp on CPU, loopback-only Ollama/LM Studio adapters, basic settings, and TXT/VTT/SRT export. Windows stores local vocabulary in JSON settings and passes normalized terms to Whisper.net as local decoder prompt hints. Its Whisper model catalog is now revision-pinned and SHA-256 verified, and CI runs real tiny-model ASR and S1-mini cleanup with a network-denying model-store client after provisioning. Linux has matching editable Recognition Hints, persisted in TOML and passed as whisper.cpp's local initial prompt, alongside an in-process CPU llama.cpp runtime, revision-pinned and SHA-256-verified cleanup model lifecycle, a cleanup toggle, raw-ASR fallback, and a real-model integration test. On PR head `5deaf9a`, all five CI jobs pass, including model-backed S1-mini CPU inference on all desktop platforms and Windows ASR with local weights. End-to-end airplane-mode dictation across each platform's ASR, cleanup, and insertion still needs acceptance coverage. The PR still needs its required human approval. The Claude review action failed internally before producing a review, and Vercel reports an account-level integration failure. The release workflow builds versioned releases and a serialized rolling `latest` prerelease. Native installers and the update path remain incomplete.
...
| Transcript cleanup | Embedded S1-mini GGUF through llama.cpp on macOS/Linux and CPU LLamaSharp on Windows; optional loopback Ollama/LM Studio on macOS/Windows; loopback Ollama YAP on Linux | Complete UX parity for tone/format options, tested model lifecycle and airplane-mode behavior; raw-ASR fallback | P0 |
...
| Insertion | Accessibility-based paste; macOS paste is asynchronous and restores all captured pasteboard data types | Permission onboarding, app-specific insertion fallback, undo/recovery | P0 |
