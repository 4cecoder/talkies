...
The macOS app has a menu-bar entry, a floating dictation window, selectable Left or Right Option activation with press/hold detection, AVFoundation recording, WhisperKit transcription with local personal-vocabulary hints, cursor insertion with asynchronous paste and multi-representation clipboard restoration, opt-in in-process S1-mini cleanup through llama.cpp on CPU, loopback-only Ollama/LM Studio adapters, basic settings, and TXT/VTT/SRT export. Windows stores local vocabulary in JSON settings and passes normalized terms to Whisper.net as local decoder prompt hints; it also has an embedded CPU S1-mini provider with revision-pinned download, SHA-256 verification, license notices, and first-use progress. Linux has matching editable Recognition Hints, persisted in TOML and passed as whisper.cpp's local initial prompt, alongside an in-process CPU llama.cpp runtime, revision-pinned and SHA-256-verified cleanup model lifecycle, a cleanup toggle, raw-ASR fallback, and a real-model integration test. On PR head `2add232`, all five CI jobs pass, including model-backed S1-mini CPU inference on all three desktop platforms; Windows inference now uses a network-denying HTTP handler after model verification. End-to-end airplane-mode dictation across ASR, cleanup, and insertion still needs acceptance coverage. The PR still needs its required human approval. The Claude review action failed internally before producing a review, and Vercel reports an account-level integration failure. The release workflow builds versioned releases and a serialized rolling `latest` prerelease. Native installers and the update path remain incomplete.
