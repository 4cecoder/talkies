...
The macOS app has a menu-bar entry, a floating dictation window, selectable Left or Right Option activation with press/hold detection, AVFoundation recording, WhisperKit transcription, cursor insertion with asynchronous paste and multi-representation clipboard restoration, opt-in in-process S1-mini cleanup through llama.cpp on CPU, loopback-only Ollama/LM Studio adapters, basic settings, and TXT/VTT/SRT export. Windows has an embedded CPU S1-mini provider with revision-pinned download, SHA-256 verification, license notices, and first-use progress, alongside loopback-only Ollama and LM Studio. Linux now has an in-process CPU llama.cpp runtime, revision-pinned and SHA-256-verified model lifecycle with license notices, a cleanup toggle, raw-ASR fallback, and a real-model integration test. All platform builds and real S1-mini inference checks passed on cleanup PR head `9e17a0a`; the PR still needs its required approval. The release workflow now builds versioned releases and a serialized rolling `latest` prerelease. Native installers and the update path remain incomplete.
