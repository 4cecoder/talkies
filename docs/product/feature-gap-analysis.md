...
The macOS app has a menu-bar entry, a floating dictation window, Right Option activation with press/hold detection, AVFoundation recording, WhisperKit transcription, cursor insertion, local Ollama/LM Studio adapters, basic settings, and TXT/VTT/SRT export. Windows and Linux have independent apps. The repository has platform unit/integration tests and a tag-based release workflow, but the coverage and artifact quality are uneven.
...
| Transcript cleanup | Optional Ollama/LM Studio local HTTP server; no embedded S1-mini path | Built-in local S1-mini, first-class toggle, clear fallback to raw ASR | P0 |
| Swift implementation | SwiftUI shell and WhisperKit; no package volatility boundary; Swift tools version 6.0 | Swift 6.3+ package with stable domain/UI and isolated model/runtime targets | P0 |
...
The card's trained control line accepts four style values (`casual`, `semi-casual`, `semi-formal`, `formal`), two structures (`prose`, `lists`), and two contexts (`general`, `email`). The Superwhisper announcement describes a five-position tone control including `balanced`; that UI value is not listed in the model card's accepted control values. Talkies should expose only tested values until an exact mapping is verified. The prompt must use the model card's exact system prompt and disable Qwen3 thinking. Blank output for filler-only speech is valid; preserve raw ASR as a safe fallback.

## Delivery sequence

1. Make the current macOS app build and launch on Swift 6.3+, then separate stable and frequently changing model/runtime code.
2. Add a Swift-owned local cleanup adapter and S1-mini model lifecycle, with golden transcript fixtures for cleanup behavior.
