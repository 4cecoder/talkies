...
The macOS app has a menu-bar entry, a floating dictation window, selectable Left or Right Option activation with press/hold detection, AVFoundation recording, WhisperKit transcription with local personal-vocabulary hints, cursor insertion with asynchronous paste and multi-representation clipboard restoration, opt-in in-process S1-mini cleanup through llama.cpp on CPU, loopback-only Ollama/LM Studio adapters, basic settings, and TXT/VTT/SRT export. Windows now stores local vocabulary in its JSON settings and passes the normalized terms to Whisper.net as local decoder prompt hints; it also has an embedded CPU S1-mini provider with revision-pinned download, SHA-256 verification, license notices, and first-use progress. Linux now has an in-process CPU llama.cpp runtime, revision-pinned and SHA-256-verified model lifecycle with license notices, a cleanup toggle, raw-ASR fallback, and a real-model integration test. Linux vocabulary controls and prompt conditioning remain to be implemented. All platform builds and real S1-mini inference checks passed on cleanup PR head `2af9f1b`; the PR still needs its required approval. The release workflow now builds versioned releases and a serialized rolling `latest` prerelease. Native installers and the update path remain incomplete.
...
| Vocabulary and replacements | macOS/Windows local terms bias Whisper decoding; no Linux dictionary workflow or cleanup integration | Shared local terms, pronunciations, replacements, import/export, use in ASR and cleanup prompts | P1 |
...
2. Keep verified S1-mini cleanup on macOS, Windows, and Linux; close Linux workflow verification and provide matching controls and offline acceptance coverage.
