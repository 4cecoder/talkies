...
- Verified on 2026-09-24 at PR head `c2ab534`: all four project CI jobs pass. Linux builds with Zig 0.17-dev and runs pinned S1-mini cleanup and pinned Whisper tiny CPU ASR inference; Windows builds and tests cached Whisper ASR and S1-mini CPU inference with model-store requests denied during inference; macOS builds, runs package tests, verifies pinned S1-mini CPU inference and WhisperKit tiny ASR from a model folder with downloads disabled, and smoke-tests the app bundle; the website passes typecheck, lint, Pages static export, and route/asset verification. Windows also has fast model-store tests for verified downloads, bad hashes, wrong sizes, partial cleanup, and atomic replacement. Both Windows and Linux Whisper downloads pin a repository revision and verify expected size and SHA-256.
