...
The CI builds and tests the actual macOS Swift package, runs native Windows and Zig/Linux build-and-test jobs, and checks the frontend on pull requests. macOS CI provisions the pinned S1-mini GGUF and WhisperKit tiny model, then runs an integrated cached-model acceptance test through ASR, S1-mini cleanup, and `TextInserter` with a mocked Accessibility client. WhisperKit is initialized with downloads disabled, and the cleanup model store rejects URLSession requests during inference. Windows and Linux currently test their local ASR and cleanup stages separately; integrated acceptance through insertion remains planned there. A full app test under an OS-level block on all external networking also remains planned. Release builds validate platform packages and publish SHA-256 checksums. Models are provisioned before the offline acceptance step.
